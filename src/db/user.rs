...
impl UserRepository {
    ...
    pub async fn store_bundle(&self, bundle: Identity) -> Result<Identity> {
        //println!("{:?}",format!("{:?}",bundle.identity_key));
        let bundle1 = sqlx::query_as::<_, Identity>("insert into yolo_bundle (name_ ,identity_key,signed_pre_key,signature_,one_time_pre_key) values ($1,$2,$3,$4,$5) returning *")
    ...
    pub async fn get_stored_bundle_of(&self , name_ : String) -> Result<Identity>
    {
    ...
    println!("{:?}",name_.clone());
       ...
       // println!("{:?}", vec);
        ...
        let bundle = Identity {
            name_ : bundle_to_return_stringify.name_,
            identity_key: parse_bundle_arguments(bundle_to_return_stringify.identity_key),
            signed_pre_key : parse_bundle_arguments(bundle_to_return_stringify.signed_pre_key),
            signature : parse_bundle_arguments(bundle_to_return_stringify.signature_),
            one_time_pre_key: parse_bundle_arguments(bundle_to_return_stringify.one_time_pre_key),
        };
        println!("{:?}", bundle);
        Ok(bundle)
